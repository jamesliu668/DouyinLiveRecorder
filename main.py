    ...
    with tqdm(total=24*3600, desc="正在录制中", leave=False) as pbar:
        while True:
            try:
                ...
                logging.warning(f"错误信息: {e} 发生错误的行数: {e.__traceback__.tb_lineno}")
    ...
    try:
        ...
        if record_url.find("https://live.douyin.com/") > -1:
            ...
            with semaphore:
                ...
                json_data = get_douyin_stream_data(record_url, cookies=dy_cookie, ua=ua.random)
            ...
            logging.warning(f'{record_url} 未知直播地址')
            return
...
if __name__ == '__main__':
    ...
    # logger.setLevel(logging.DEBUG) # 默认是INFO
    formatter = logging.Formatter('%(asctime)s | %(levelname)s | %(module)s.%(funcName)s:%(lineno)d | %(message)s')

    logFileName = os.path.join(logFolder, 'Douyin.log')
...
try:
    ...
    if os.path.exists(sourceFile):
        ...
        record_thread = threading.Thread(target=start_record, args=url_tuple)
